...
export class AddBlogComponent implements OnInit {
  ...
  constructor(private usuarioService:UsuarioService, private blogService:BlogService, private imagenService:ImagenService, private snack:MatSnackBar, private router:Router) { }
  ...
  crearBlog() {
    if (this.blog.titulo == '' || this.blog.titulo == null) {
      ...
      this.snack.open('La fecha del blog es requerido **', '', {
        duration: 2000,
      });
    } else if (this.blog.imagen == '' || this.blog.imagen == null) {
      this.snack.open('La imagen del blog es requerido **', '', {
        duration: 2000,
      });
    
    } else {
      this.blogService.crearBlog(this.blog).subscribe(
        (data) => {
          swal.fire({
            position: 'center',
            icon: 'success',
            title: 'Se creó el blog con éxito',
            showConfirmButton: false,
            timer: 1000,
          });
          this.reset();
