...
export class ActualizarCursoComponent implements OnInit {
  ...
  constructor(private cursoService: CursoService, private imagenService: ImagenService, private modalidadService: ModalidadService, private categoriaService: CategoriaService, private docenteService: DocenteService, private usuarioService: UsuarioService, private snack: MatSnackBar, private router:Router, private activatedRoute:ActivatedRoute) { }
  idUser: any;
  ngOnInit(): void {
    ...
    this.cursoService.listarCursos().subscribe(
      data =>{
        ...
        this.listaCursos.find((cursoDeLista:any)=>{
        if(cursoDeLista.idCurso = this.idCurso){
          this.curso = cursoDeLista;
        }
        })
      }
    );
      ...
      this.cursoService.actualizarCurso(this.curso).subscribe(
        data =>{
          swal.fire({
            position: 'center',
            icon: 'success',
            title: 'Se Actualizó el curso con éxito',
            showConfirmButton: false,
            timer: 1000,
          });
          this.reset();
